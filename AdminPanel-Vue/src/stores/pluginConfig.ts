...
export const usePluginConfigStore = defineStore('plugin-config', () => {
  ...
  async function savePluginConfig(pluginName: string) {
    ...
    const commentEntries = configEntries.value.filter((entry) => entry.isCommentOrEmpty)

    const configString = [
      ...commentEntries.map((entry) => String(entry.value)),
      ...Array.from(allEntriesMap.values()).map((entry) => {
