...
export function useDashboardState() {
    ...
    useRequest<Awaited<ReturnType<typeof systemApi.getSystemResources>>>(
      (context) =>
        systemApi.getSystemResources(
          ...
          { showLoader: false, timeoutMs: 10000 }
        ),
      {
  ...
  const weather = ref<WeatherDisplay>({
    icon: "--",
    temp: 0,
    text: "加载中…",
    humidity: 0,
  ...
  async function loadNews() {
    try {
      const response = await newsApi.getGroupedNews(2, 10, {
        showLoader: false,
  ...
  function startPollingGroup(pollings: PollingController[]) {
    pollings.forEach((polling) => polling.start());
  }

  function stopPollingGroup(pollings: PollingController[]) {
    pollings.forEach((polling) => polling.stop());
  }
  ...
  const nonCriticalPollings: PollingController[] = [
    monitorPolling,
    logPolling,
    weatherPolling,
    newsPolling,
    newApiMonitorPolling,
  ];

  function syncPollingWithVisibility() {
    if (isPageVisible.value) {
      startPollingGroup(nonCriticalPollings);
      return;
    }

    stopPollingGroup(nonCriticalPollings);
  }
