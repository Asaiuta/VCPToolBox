...
function buildConfigFromEnvPairs(pairs: Map<string, string>): ConfigResponse {
  ...
  const jsonAgents =
    parseJsonArray<ConfigResponse["agents"] extends Array<infer T> ? T : never>(
      agentsRaw
    );
...
function parseEnvConfig(content: string): ConfigResponse {
...
function normalizeConfigPayload(payload: unknown): ConfigResponse {
...
async function loadConfigFromPluginsEndpoint(): Promise<ConfigResponse | null> {
  try {
